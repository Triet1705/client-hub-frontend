...
export function TechStackSection() {
  const technologies = [
    { 
      name: "Next.js", 
      icon: <Image src="/next.svg" alt="Next.js" width={100} height={30} className="invert opacity-40 group-hover:opacity-100 transition-opacity duration-300" /> 
    },
    { 
      name: "Supabase", 
      icon: <Image src="/supabase-logo-wordmark--dark.svg" alt="Supabase" width={120} height={30} className="opacity-40 group-hover:opacity-100 transition-opacity duration-300" /> 
    },
    { 
      name: "PostgreSQL", 
      icon: (
        <div className="flex items-center gap-3 opacity-40 group-hover:opacity-100 transition-opacity duration-300 grayscale group-hover:grayscale-0">
  ...
  return (
    <section id="proof" className="py-24 relative overflow-hidden bg-[#0A0A0A] border-t border-white/[0.05]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 mb-12">
        <div className="text-center max-w-3xl mx-auto">
          <h3 className="text-sm uppercase tracking-[0.2em] font-semibold text-slate-500 mb-8">
            TRUSTED BY TEAMS THAT SWITCHED FROM LEGACY STACKS
          </h3>
        </div>
      </div>
      ...
      <div className="w-full relative py-8 overflow-hidden">
        ...
        <div className="flex gap-16 md:gap-24 whitespace-nowrap animate-[scroll_40s_linear_infinite] w-max hover:[animation-play-state:paused] items-center">
          {duplicatedTechnologies.map((tech, i) => (
             <div key={i} className="group flex items-center justify-center min-w-[120px] cursor-default">
               {tech.icon}
             </div>
          ))}
        </div>
