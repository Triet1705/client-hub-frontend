...
function ProofStatus({ status }: { status: AuditVerificationStatus }) {
  const config = {
    VERIFIED: { icon: ShieldCheck, label: "Verified against the on-chain root", className: "text-emerald-300" },
    PENDING: { icon: Clock3, label: "Proof created; blockchain confirmation is pending", className: "text-amber-300" },
    NOT_ANCHORED: { icon: Clock3, label: "This record is waiting for an anchor batch", className: "text-content-muted" },
    TAMPERED: { icon: ShieldAlert, label: "Verification failed: stored content or proof does not match", className: "text-rose-300" },
    CHAIN_UNAVAILABLE: { icon: Unplug, label: "The blockchain is unavailable; local proof remains stored", className: "text-sky-300" },
  }[status];
